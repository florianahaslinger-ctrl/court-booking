...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: club } = await admin.from("clubs")
      .select("id,slug,name,currency,stripe_enabled,stripe_account_id").eq("slug", club_slug).maybeSingle();
    ...
    const body =
      `mode=payment` +
      `&metadata[type]=topup&metadata[club_id]=${club.id}&metadata[email]=${encodeURIComponent(mail)}&metadata[amount]=${eur}` +
      ...
      `&line_items[0][price_data][product_data][name]=${encodeURIComponent(`Guthaben-Aufladung ${club.name}`)}` +
