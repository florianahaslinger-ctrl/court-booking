-- ============================================================
-- Migration 16 – Prozent-Rabatt für Mitglieder wieder aktiv
--
-- Migration 12 hatte den Member-Faktor auf "binär" gesetzt (gratis=0,
-- sonst 1) und damit den Modus 'discount' (z. B. Indoor 50 %) faktisch
-- ignoriert -> Mitglieder zahlten Indoor den vollen Preis, obwohl Admin-UI
-- und Konto-Texte einen Rabatt versprachen.
--
-- Jetzt gilt für den Mitglieder-Anteil je Platzart:
--   free      -> 0
--   discount  -> (100 - discount_percent) / 100     (z. B. 50 % -> 0,5)
--   full      -> 1
-- Das Split-Modell bleibt: Gesamt = Platzpreis * Summe(Faktoren) / Personen.
-- (Nur der create_booking-Body wird ersetzt; Signatur unverändert.)
-- ============================================================
...
begin
  ...
  -- Mitglieder-Faktor: gratis=0, Rabatt=(100-d)/100, sonst voller Anteil=1
  ...
  if v_price = 0 then
    ...
    v_status := 'confirmed'; v_payment := 'paid'; v_hold := null;      -- per Guthaben bezahlt
  else
  ...
  -- Guthaben abbuchen (atomar; bei zu wenig -> Exception -> Rollback der Buchung)
...
grant execute on function public.create_booking(text,uuid,timestamptz,timestamptz,text,text,jsonb,boolean) to anon, authenticated;
