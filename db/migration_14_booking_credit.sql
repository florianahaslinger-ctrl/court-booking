-- ============================================================
-- Migration 14 – Buchung wahlweise per Guthaben bezahlen
-- Neuer Parameter p_use_credit: wenn true und Preis>0, wird der Betrag
-- sofort vom Guthaben der Zahler-E-Mail abgebucht (Buchung bestätigt,
-- keine Stripe-Zahlung). Reicht das Guthaben nicht -> Fehler (Client
-- fällt dann auf Kartenzahlung zurück). Preislogik unverändert (reine Teilung).
-- Ersetzt die 7-Argument-Version (6/7-Argument-Aufrufe nutzen die Defaults).
-- ============================================================

drop function if exists public.create_booking(text,uuid,timestamptz,timestamptz,text,text,jsonb);
...
begin
  ...
  v_fac_member := case when v_mode = 'free' then 0 else 1 end;   -- binär (kein Prozent-Rabatt)
